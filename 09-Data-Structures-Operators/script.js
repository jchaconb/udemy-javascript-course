...
const convertSnakeToCamelCase = function () {
  const lines = inputTextarea.value.split('\n');

  for (const [index, line] of lines.entries()) {
    const [firstWord, secondWord] = line.toLowerCase().trim().split('_');
    const camelCaseVariable = `${firstWord}${secondWord.replace(
      secondWord[0],
      secondWord[0].toUpperCase()
    )}`;

    console.log(camelCaseVariable.padEnd(20) + '✅ '.repeat(index + 1));
  }
};

convertButton.addEventListener('click', convertSnakeToCamelCase);
...
console.log(airlineA.toLowerCase()); // tap air portugal
console.log(airlineA.toUpperCase()); // TAP AIR PORTUGAL

const name = 'jOnAS';
console.log(name); // jOnAS

const nameLower = name.toLowerCase();
console.log(nameLower); // jonas
...
console.log(planeB.startsWith('Air')); // false
console.log(planeB.startsWith('A320')); // true

const planeC = 'Airbus A320neo';

if (planeC.startsWith('Airbus') && planeC.endsWith('neo')) {
